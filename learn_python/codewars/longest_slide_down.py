...
# Egypt at the time. For example, lets consider the following problem.
...
# 8 5 \9\ 3
# Here comes the task...
# Lets say that the slide down is the maximum sum of consecutive numbers
...
# (in ruby: longest_slide_down) that takes a pyramid representation
...
def longest_slide_down(pyr):
    for row in range(len(pyr) - 1, 0, -1):
        for i in range(0, row):
            pyr[row - 1][i] += max(pyr[row][i], pyr[row][i + 1])
    return pyr[0][0]
